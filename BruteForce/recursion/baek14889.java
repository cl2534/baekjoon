...
public class baek14889 {
    static int [][] s;
    static int n;
    static int recur(ArrayList<Integer> first, ArrayList<Integer> second, int index) {
        if (index == n) {
        ...
        int ans = -1;
        first.add(index);
        int res1 = recur(first, second, index + 1);
        if (ans == -1 || res1 != -1 && ans > res1) {
            ans = res1;
        }
        first.remove(first.size() - 1);
        second.add(index);
        int res2 = recur(first, second, index + 1);
    ...
    public static void main(String [] args) {
        ...
        System.out.println(recur(first, second, 0));
    }    
}